...
- **Clinical NER** via Hugging Face ([`d4data/biomedical-ner-all`](https://huggingface.co/d4data/biomedical-ner-all) by default)
- **Assertion detection** with [medspaCy](https://github.com/medspacy/medspacy) ConText (negation, history, family history)
- **Terminology linking** to ICD-10 and RxNorm using [SapBERT](https://huggingface.co/cambridgeltl/SapBERT-from-PubMedBERT-fulltext) + lexical reranking
...
Then open http://localhost:5173. The API lives at http://localhost:8000 (interactive docs at http://localhost:8000/docs).
...
| **[Synthea](https://synthea.mitre.org/)** | 12 bundled FHIR patient bundles in `backend/app/data/synthea/` |
...
There is a separate training track for fine-tuning PubMedBERT on the public [NCBI-Disease](https://huggingface.co/datasets/ncbi/ncbi_disease) corpus and benchmarking it against the live baseline. Metrics land in `backend/eval/finetune_metrics.json` and show up in the Evaluation tab.
